...
(function($, document, Math, devicePixelRatio) {
  ...
  peity.register(
    ...
    function(opts) {
      ...
      var strokeBuffer = (opts.strokeWidth - 1)
      var yQuotient = (height - strokeBuffer) / (max - min)

      var coords = [];
      var i;

      context.beginPath();
      context.moveTo(0, height + (min * yQuotient) + (strokeBuffer / 2))

      for (i = 0; i < values.length; i++) {
        var x = i * xQuotient
        var y = height - (yQuotient * (values[i] - min)) - (strokeBuffer / 2)
